...
Four of those contract cases pin the definition-write semantics, because they are provider
obligations rather than AWS details: an upsert over a live row **preserves all four `last_run_*`
fields**, it **keeps guard 4 armed** (an outcome for an older fire is still rejected afterwards), an
upsert onto a soft-deleted row **raises `SchedulerConflictError` and leaves the row deleted**, and an
upsert carrying a superseded `scheduled_task_version` **raises and writes nothing**.
...
18. **`upsert` over an existing row writes only the definition fields, through `update_fields`**
    (`providers/aws.py`). `put` is now reached on row *creation* only, which is what the
    *Concurrency contract* always specified. *Intentional:* the pre-change whole-row write reset
    every field the definition object does not carry — all four `last_run_*`, plus `status` and
    `completed_at` — so a routine idempotent re-create erased run history and, by clearing
    `last_run_scheduled_time`, disarmed guard 4 so an *older* in-flight fire was then accepted as
    the newest run. It also reverted any outcome recorded since the caller read the row. Two new
    409 paths fall out, both listed in the error table: an upsert onto a soft-deleted row, and a
    definition write whose `scheduled_task_version` no longer matches.
19. **Both stream runners route a record carrying `scheduled_run` to the non-stream runner**
...
- Agent-runner happy path on both targets. Both stream runners for **interactive** traffic: their
  ...
  carrying a `scheduled_run` block takes the new non-stream branch (behavioural change #19).
...
| Row write succeeds, timer registration fails | `Scheduler.upsert` | The row is restored to its prior state (removed outright when it was newly created), then the error propagates. A row without a registration would silently never fire, which is worse than a failed create. Each path rolls back through the same primitive it wrote with — `remove` after a `put`, a definition field-write after a definition field-write — so a rollback can never reset run history or resurrect a tombstone |
| Upsert target is soft-deleted | `AWSScheduler.upsert` | `SchedulerConflictError` → 409, **before any write or AWS call**. The service already rejects a deleted id, so this is reachable only when a delete lands between that check and the provider's own read; writing on through would put definition fields on a tombstone and register a timer for a deleted task |
| Definition write loses a race — row vanished, or deleted and recreated | `AWSScheduler._apply_definition` | `SchedulerConflictError` → 409. The write is conditioned on `scheduled_task_version`, so a `False` return means the row is gone or a successor incarnation now holds the id. Failing is the point: writing on would either resurrect a deleted row or overwrite a different scheduled task's definition |
