...
- Enabled by mounting the handler at the user level (thread precedent). Surfaces:
  ...
  - Single-process pipeline topology: `RESTAPI.run()` delegates to `IOHandler` only when no explicit handlers are passed (`api/http.py`, activation rule), so the pipeline `RequestHandler`/`IOHandler` composes the schedule routes automatically when the `schedule` block is configured, with the `Authoriser` injectable at that entry point.
