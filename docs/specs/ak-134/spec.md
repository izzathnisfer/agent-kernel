...
3. **An unbuildable mapping store fails at startup, never at request time.** Because `SessionStore.get_mapping_store()` is `@abstractmethod` (§New package, governing rule 5), a mapping store can only fail to materialize while `SessionStoreBuilder` constructs the *session* store — a bring-your-own session store that omits the method raises `TypeError`, a missing `session.<backend>` block raises `ValueError`, and a missing optional extra raises `ImportError`. All three surface before any request is served, so the feature needs no runtime degrade path, no cached warning, and no intent-split between auto-enabled and explicitly-enabled deployments — and none exists. An earlier revision had exactly that machinery (an `_auto_disabled` cached-WARNING degrade plus an `InitiationQueueDispatcher.register()` warm-up); both were deleted in `3b88c83f` when the abstract pairing superseded them, and the mapping-store config key that was their other trigger has since been removed too (§Config changes).
...
├── base.py       # SessionStore (ABC, + get_mapping_store) and MappingStore (ABC) — no config/factory imports
...
class InitiationManager:
    ...
    ak-py/src/agentkernel/core/thread/manager.py:78). None when conversation_initiation_enabled is False."""
    @classmethod
...
4. **Governing rule — the mapping store belongs to the session store:** there is no separate backend selection and **no mapping-store config key**. Each session store constructs its paired `MappingStore` directly in its own constructor and returns it from `get_mapping_store()`, so `session.type` selects both at once and they cannot drift apart. A bring-your-own mapping store therefore needs no setting of its own: it arrives with a bring-your-own session store (rule 5 forces one), or by subclassing a built-in store and overriding the method. An earlier revision had a `session.initiation.store` dotted path; it was removed as redundant — and as actively misleading, since a BYO session store implements `get_mapping_store()` itself and would silently ignore the key. Connection settings come from the existing `session.<backend>` config (`_SessionStoreConfig`, url/credentials); namespace (table/collection name or key prefix) is derived by suffixing the session store's own (`-id-mapping` / `id-mapping:`), and TTL is reused from it — no separate namespace config. `InitiationManager` never builds storage: it takes the store from `Runtime.current().sessions().get_mapping_store()`.
...
- **redis / valkey**: `RedisDriver` (`ak-py/src/agentkernel/core/util/driver/redis.py:8`) / `ValkeyDriver` (`driver/valkey.py:8`) — lazy connect, ping-reconnect, and retry lifecycle inherited from `_RedisLikeDriver` (`driver/redis_like.py:32`). Instantiated as the session stores do (e.g. `session/valkey.py:26`), reusing `session.redis`/`session.valkey` wholesale: `url`/`ttl` as-is, `prefix` suffixed with `id-mapping:`. Records via `driver.set(key, value)` / `driver.get(key)`; save-if-absent is check-then-act in `InitiationManager.bind()`, uniform across backends (`set(..., nx=True)` remains available should an atomic variant ever be needed).
- **dynamodb**: `DynamoDBDriver(table_name=f"{session.dynamodb.table_name}-id-mapping", partition_key="map_key", ttl=session.dynamodb.ttl)` (`driver/dynamodb.py:23`) — `sort_key` stays `None`, which the driver supports (key built from the partition key alone, `driver/dynamodb.py:79-81`), fitting the hash-only table; TTL > 0 attaches the `expiry_time` attribute on put, matching the Terraform TTL attribute.
- **firestore**: `FirestoreDriver` (`driver/firestore.py:12`) with `collection_name=f"{session.firestore.collection_name}-id-mapping"`, `project_id`/`database_id`/`ttl` from `session.firestore` as-is; one document per record key.
- **cosmosdb**: `CosmosDBDriver(connection_string=session.cosmosdb.connection_string, table_name=f"{session.cosmosdb.table_name}-id-mapping")` (`driver/cosmosdb.py:23`); no TTL (matches `CosmosDBThreadStore`'s no-TTL support).
...
- Integration handlers wrap their existing derivation with `self.resolve_session_id(...)` at exactly these points: Slack `slack_chat.py:138` (resolve `thread_ts` — DMs and channels are treated identically, no DM-specific fallback: a reply must be threaded to continue an initiated conversation, since an un-threaded reply's own ts can't be told apart from a reply to any other prior conversation in the same DM; a miss keeps the platform-derived id, starting a new session), WhatsApp `whatsapp_chat.py:278`, Telegram `telegram_chat.py:185`, Messenger `messenger_chat.py:195`, Instagram `instagram_chat.py:213`, Teams `teams_chat.py:143` (resolve `conversation_id`), Gmail `gmail_chat.py:267` and `:413`. `AgentGmailRequestHandler` is not a `RESTRequestHandler` (`gmail_chat.py:23`), so it inherits `SessionIdResolver` directly.
...
class _SessionInitiationConfig(BaseModel):
    ...
    enabled: Optional[bool] = Field(
        default=None,
        description="None (default) auto-enables in queue mode; explicit true/false overrides.",
    )
...
- The block is nested inside `session:` rather than sitting at the top level, because the feature's storage is the session store's own — one connection, one namespace, one place to configure both. There is no top-level `conversation_initiation:` block. The block carries only the feature gate (`enabled`) and the per-capability `agents` scope; the mapping store itself is not configurable here (§Design rule 4).
...
- `test_mapping_store.py` — `InMemoryMappingStore` bidirectional save/get/clear and save-if-absent idempotency; each session backend pairs itself with the matching mapping store and derives namespace/TTL from its own settings (monkeypatch `agentkernel.core.config.AKConfig.get` with a `FakeCfg` exposing `session.type` + per-backend blocks + the nested `session.initiation` block, per the established pattern); `session.initiation.store` overrides the pairing; `test_store_builders.py` covers the two ways a mapping store can arrive (a built-in session store supplying its pair, and a BYO session store returning its own) and that a session store omitting the abstract `get_mapping_store()` raises `TypeError` as the builder constructs it; `InitiationManager.get()` returns `None` when `conversation_initiation_enabled` is False and a manager when True (`InitiationManager.reset()` in an autouse fixture, mirroring thread-manager tests). `TestExtraIsolation` pins the extras contract from §New package: with `redis` blocked in `sys.modules` (`monkeypatch.setitem`, the idiom used by `test_guardrail.py`/`test_trace.py`), `agentkernel.core.session.valkey` still imports and never pulls in `core.util.driver.redis`; the purge uses `monkeypatch.delitem` so the original module objects are restored on teardown, since other tests assert class identity against their own top-level imports.
...
- `test_slack_session_resolution.py` / `test_integration_session_resolution.py` (post-review addition) — inbound session-id derivation driven end-to-end through each handler's own inbound-message method for every messaging integration except Teams (the one exception, tracked as a known gap): Slack, WhatsApp, Telegram, Messenger, Instagram, and both of Gmail's resolution points (`_process_email`, `_process_with_agent`).

Existing test files: `test_akresponsehandler.py` must pass unchanged (the new branch triggers only on the new attribute); `test_config.py` gains assertions covering `conversation_initiation_enabled`'s auto-detect/explicit-override behavior. No existing patch targets move.
