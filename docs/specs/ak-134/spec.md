...
- Integration handlers wrap their existing derivation with `self.resolve_session_id(...)` at exactly these points: Slack `slack_chat.py:125` (resolve `thread_ts`, plus a DM fallback: a DM reply can be top-level — its own ts — or threaded under the bot message's ts, so no single ts round-trips; when the `thread_ts` lookup misses and `channel_type == "im"`, the DM channel id is resolved too, and a double miss keeps the platform-derived id), WhatsApp `whatsapp_chat.py:277`, Telegram `telegram_chat.py:183`, Messenger `messenger_chat.py:193`, Instagram `instagram_chat.py:211`, Teams `teams_chat.py:141` (resolve `conversation_id`), Gmail `gmail_chat.py:265` and `:411`. `AgentGmailRequestHandler` is not a `RESTRequestHandler` (`gmail_chat.py:23`), so it inherits `SessionIdResolver` directly.
