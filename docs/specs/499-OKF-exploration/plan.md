...
- **Goal:** Runnable interactive demo with the three role agents and a committed bundle for offline runs.
- **Files:** `examples/cli/okf/openai/demo.py`, `examples/cli/okf/openai/sample_bundle/` (e.g.
  `sales/{index.md, log.md, tables/orders.md, …}`).
- **Steps:**
  1. Build `OKFBundle` on `FileSystemStorage` pointed at `sample_bundle/` (the no-AWS default);
     document the `S3Storage` swap as the only local↔cloud difference.
  ...
  4. Author `sample_bundle/` as a small valid OKF tree (doubles as format documentation).
- **Verify:** `uv run python demo.py` locally answers a Consumer question grounded in `sample_bundle/`.
