...
- **Steps:**
  ...
  2. `write_concept`: guardrail-gated create/replace → persist → cache update → **regenerate the
     touched directory's `index.md` and every ancestor index up to the bundle root** as flat
     mechanical listings, so a fresh subtree is reachable from the root (design → invariant note).
...
- **Steps:**
  1. Build `OKFBundle` on `FileSystemStorage` pointed at `sample_bundle/` (the no-AWS default —
     ...
     difference.
  2. Define Consumer / Producer / Curator `Agent`s differing only in prompt and the tool subset —
     each tool wrapped with the OpenAI Agents SDK's `function_tool` and the selected subset passed to
     the `Agent`'s `tools=` (deliberate over `OpenAIToolBuilder.bind`; design → Agent-facing tools).
     The tool split is the permission model; only the Curator gets source tools, read-only.
