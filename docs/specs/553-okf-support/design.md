...
Splits the flat `KnowledgeBase` ABC into a typed hierarchy (`VectorKB` / `StructuredKB` / `FileSystemKB`), adds an OKF-aware backend (`OKFManager`) over a pluggable `OKFStorage` blob-store primitive (local disk, S3), and replaces `KnowledgeBuilder`'s generic `read_kb`/`write_kb` tool pair with dynamically-named tools generated per registered backend **instance** (the two shared discovery tools, `get_schemas`/`get_all_kb_descriptions`, stay), so the agent gets a distinctly-named, distinctly-shaped tool per knowledge source instead of routing through one generic `read_kb(backend, query)`.
...
- The three children live in `knowledgebase/base.py` alongside the parent (they are part of the same contract, not separate backends). Package-level exports are **introduced** — `knowledgebase/__init__.py` currently exports nothing (a bare comment; users import from submodules, cf. `examples/cli/knowledgebase/openai/multi/demo.py:14-17`): it gains `KnowledgeBase`, the three children, and `KnowledgeBuilder`. Concrete backends (`ChromaManager`, `Neo4jManager`, `StarburstManager`, `OKFManager`) stay submodule imports so `__init__` never eagerly imports optional dependencies (the `core/util/driver/__init__.py` convention); existing submodule import paths keep working.
- `VectorKB` and `StructuredKB` are pure capability tags: **neither widens the parent's abstract contract** — a backend subclassing them implements the same five abstract members it does today. This is deliberate, not deferred:
  - Per-instance tool generation (below) reaches vector and structured backends only through the parent surface (`read(query, limit)`/`write(records)`); the children exist for classification (`isinstance` dispatch when selecting a backend's tool profile, docs, schema generation), so a per-type behavioral contract would have no caller.
...
- `FileSystemKB` is the one child **with** abstract members: `index() -> str` and `list(prefix: str = "") -> list[Record]`, on top of the parent's five. This is the trigger rule below applied at birth — the per-instance tool generator is a consumer that must call path-shaped operations (the `index_*`/`list_*` tools) polymorphically on any file/bundle backend. `exists`/`last_modified` stay `OKFManager`-only (no tool needs them).
- **Trigger rule for future contracts**: a capability child gains abstract members only when the first framework-side consumer appears that must call capability-specific behavior polymorphically (`FileSystemKB.index`/`list` above is this rule's first application). For `VectorKB`/`StructuredKB` the visible candidates are the deferred OKF→`VectorKB` ingestion pipeline (a non-goal here, would need an upsert-shaped method) and a generic RAG pre-hook over all vector KBs (would need scored semantic search beyond `read(query, limit)`). When that day comes, prefer declarative members (e.g. `StructuredKB.query_language: str`) over operational ones, and note that adding abstract members is a breaking change for third-party subclasses already tagged with the child.
- Reclassify the three existing backends (declaration-only change; no behavior change to their `read`/`write` logic):
  ...
  - `Neo4jManager(VectorKB, StructuredKB)` — **multiple inheritance**: Neo4j-the-database genuinely has both vector-index and Cypher capability; MRO is a plain diamond back to `KnowledgeBase` since neither parent adds conflicting members. (The class's current implementation is Cypher-only on both paths — see Open questions on whether the `VectorKB` tag ships now.)
...
- The new concrete backend is `OKFManager(FileSystemKB)` — named consistently with `ChromaManager`/`Neo4jManager`/`StarburstManager`. `FileSystemKB` stays open for future non-OKF file backends (each implements the same `index`/`list` members).
...
One concrete class, `OKFManager(FileSystemKB)`, parametrized by an `OKFStorage` instance (composition — `LocalOKFStorage`/`S3OKFStorage` supply the difference; no `LocalOKFManager`/`S3OKFManager` subclasses; when the `storage` constructor argument is omitted it is resolved from config — see Config-driven wiring). It implements OKF v0.2 semantics on top of the blob surface (spec details in `research/okf-spec-summary.md`):
...
- **Progressive disclosure** — `index()` (the `FileSystemKB` contract member) returns the bundle-root `index.md` body when present, otherwise a listing **synthesized** from concept frontmatter (`* [title](path) - description`), which the spec explicitly permits. This is the agent's intended entry point into a bundle.
- **`list(prefix)`** (the `FileSystemKB` contract member) returns concept entries (`concept_id`, `type`, `title`, `description`) rather than bare paths; **`exists(path)` / `last_modified(path)`** are `OKFManager`-only extras beyond any contract (delegating to storage).
- **Spec-mandated tolerances** (MUST rules, §11/§6.1): never validate or follow the cross-link graph (broken links are legal); tolerate unknown `type` values and unknown frontmatter keys; normalize a bare `verified` mapping to a single-element list; tolerate a missing `index.md`; attempt best-effort consumption when `okf_version` (bundle-root `index.md` frontmatter) is newer than 0.2.
- **Unknown-key preservation** (spec SHOULD, §4.1) — **value-level, not byte-level**: when a rewrite must touch the frontmatter (the `generated` stamp), unknown keys and their values survive the pyyaml parse/re-serialize cycle, but formatting, quoting, and comments do not (pyyaml cannot round-trip them; `ruamel.yaml` could, and is deliberately not added — no new dependency for a spec SHOULD). When no frontmatter change is needed (validation passes and `generated` is already present), the document is written verbatim.
...
- Replace the fixed `read_kb(backend, query, limit)` / `write_kb(backend, ...)` pair with tools generated **per registered backend instance**, dynamically named and shaped by that backend's capability profile (below).
...
- Generated tools are **capability-profile wrappers, not raw bound methods** — a tool's LLM-facing signature intentionally differs from the backend method behind it (`write_ChromaDB(text, source)` fronts `write(records)`). Each capability child defines a fixed tool profile; `KnowledgeBuilder` selects profiles by `isinstance` on each registered instance (this dispatch-on-classification is exactly what the capability children exist for):
  - `VectorKB` → `read_{name}(query, limit)`; `write_{name}(text, source)` — the wrapper builds the `{"text", "metadata": {"source"}}` record, as today's `write_kb` text path does (`knowledgebuilder.py:154, 170`).
  - `StructuredKB` → `read_{name}(query, limit)`; `write_{name}(query, params_json)` — the wrapper builds the query/params metadata record, as today's `write_kb` query path does (`knowledgebuilder.py:151-167`).
  - `FileSystemKB` → `index_{name}()`, `read_{name}(concept)`, `write_{name}(path, content)`, `list_{name}(prefix)` — delegating to the child's `index`/`list` contract members and the parent `read`/`write`.
  - Wrappers close over the backend instance and call only the parent surface plus (for `FileSystemKB`) the child's contract members — tool generation never introspects arbitrary concrete methods.
- **Multi-capability backends** get the union of their profiles' operations, deduplicated by generated name: an operation whose name **and** signature coincide across profiles is generated once (`read_{name}(query, limit)` is identical in the vector and structured profiles); a name collision with **different** signatures is disambiguated with a capability infix. So `Neo4jManager(VectorKB, StructuredKB)` yields `read_Neo4jDB(query, limit)`, `write_vector_Neo4jDB(text, source)`, `write_query_Neo4jDB(query, params_json)`. Single-capability backends never carry an infix.
- Generated tools keep the existing error contract: backend exceptions are caught and returned as readable strings to the LLM, never raised into the framework (as `read_kb`/`write_kb` do today, `knowledgebuilder.py:125-130, 169-174`); semantic-map placeholder resolution likewise stays applied to query-shaped arguments (`knowledgebuilder.py:121, 152`).
- Tool generation uses a dynamic-function-attribute mechanism equivalent to the `DynamicToolModel` sketched in the originating proposal: given a wrapper callable, set its `__name__` (to the generated tool name), `__doc__`, and parameter annotations before handing it to the framework's `ToolBuilder.bind()` (`core/tool.py:144-162`), which — for OpenAI via `function_tool` (`framework/openai/openai.py:369-383`) — derives the tool's name, description, and parameter schema straight from those function attributes.
...
- The factory's consumer is `OKFManager` itself: its `storage` constructor parameter is optional — when omitted, the manager resolves storage from the `AKConfig` OKF section via the factory; an explicitly passed `OKFStorage` instance always wins and bypasses config entirely. No AK-internal wiring ever constructs an `OKFManager` — registration with `KnowledgeBuilder` stays in user code, like every other backend.

### Examples and docs

- New example `examples/cli/knowledgebase/openai/okf/` (`demo.py` + `demo_test.py`, per the existing per-backend example convention) — an OKF bundle over `LocalOKFStorage`, wired through `KnowledgeBuilder`.
- Rework `examples/cli/knowledgebase/openai/multi/demo.py` — its agent instructions and tool wiring are written against `read_kb`/`write_kb` (`demo.py:186-222`) and break under per-instance tools.
- Update the docs and skills surfaces that describe the fixed 4-tool surface — `docs/docs/advanced/knowledge-bases.md`, `docs/docs/core-concepts/overview.md`, `.agents/skills/ak-dev-architecture`, `.agents/skills/ak-dev-new-knowledgebase-integration`, and the bundled `ak-add-capabilities` skill — via the standard docs/skills sync flows before merge.
...
- **Neo4j `VectorKB` tag vs implementation**: `Neo4jManager`'s current implementation is Cypher-only on both paths — `read` wraps the query in a Cypher `CALL` subquery (`neo4j.py:125-146`) and `write` executes `cypher_query` metadata (`neo4j.py:110-123`), so a vector-shaped `write_vector_Neo4jDB(text, source)` record reaches `write` with no `cypher_query` and fails. Confirm whether the `VectorKB` tag (and its infixed write tool) ships now — accepting a tool with no working implementation behind it — or whether `Neo4jManager` stays `StructuredKB`-only until its vector-index support is actually implemented.
