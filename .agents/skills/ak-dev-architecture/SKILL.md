...
- **`MappingStore`** (`core/session/base.py`, beside `SessionStore`): ABC storing the `session_id ↔ messaging_integration_thread_id` association as **two records** (`thread#<id>` → session, `session#<id>` → thread id) so both directions are O(1); `THREAD_RECORD_PREFIX`/`SESSION_RECORD_PREFIX` and the `thread_record_key()`/`session_record_key()` composers live on the ABC as ClassVars/staticmethods. It is owned by the session store, not the initiation package: each backend constructs its pair directly in its constructor and returns it from `SessionStore.get_mapping_store()`, so `session.type` selects both and they share one connection. There is **no mapping-store config key and no builder** — a bring-your-own mapping store arrives with a bring-your-own session store (which `get_mapping_store()` forces), or by subclassing a built-in store and overriding the method. Namespace (table/collection name or key prefix) is derived by suffixing the session store's own (`-id-mapping` / `id-mapping:`), and TTL is reused from it. `get_mapping_store()` is `@abstractmethod`, so a backend added without its mapping counterpart cannot be instantiated — the requirement is enforced by Python at startup rather than by a runtime gate, and it is a breaking change for bring-your-own session stores predating the method. Backends reuse the shared `core/util/driver/` drivers; DynamoDB uses a hash-only table (partition key `map_key`). The Redis/Valkey-shared `_RedisLikeMappingStore` lives in its own client-library-free `core/session/redis_like.py`, **not** in `session/redis.py` — the latter imports `RedisDriver` (`import redis`), and since `redis` and `valkey` are separate extras, importing the shared class from there would break `agentkernel[valkey]`. Same split as `core/util/driver/redis_like.py`; pinned by `tests/test_mapping_store.py::TestExtraIsolation`
...
│   ├── initiation/          # Agent-initiated conversations (no storage — each backend's mapping store lives beside its session store, see session/)
...
│   └── session/             # Session store implementations, each paired with its MappingStore
│       ├── base.py           # SessionStore (+ get_mapping_store), MappingStore, SessionCache — no config imports
...
│       ├── in_memory.py      # InMemorySessionStore + InMemoryMappingStore
│       ├── redis.py          # RedisSessionStore + RedisMappingStore
│       ├── valkey.py          # ValkeySessionStore + ValkeyMappingStore (requires the `valkey` extra)
│       ├── dynamodb.py       # DynamoDBSessionStore + DynamoDBMappingStore
│       ├── cosmosdb.py       # CosmosDBSessionStore + CosmosDBMappingStore
│       └── firestore.py      # FirestoreSessionStore + FirestoreMappingStore
