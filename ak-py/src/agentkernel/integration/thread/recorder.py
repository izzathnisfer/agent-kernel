...
from ...core.model import SCHEDULED_SESSION_PREFIX, AgentRequest, BaseChatRequest
...
class ThreadRecorder:
    ...
    def pre_run(self, req: BaseChatRequest, requests: List[AgentRequest]) -> tuple[List[AgentRequest], List[ThreadAttachment]]:
        ...
        A scheduled run is a no-op: it carries no user turn to record, and its
        activity is deliberately kept out of the owner's conversation history.
        ...
        """
        if self._is_scheduled_session(req):
            return requests, []
        if not req.user_id:
    ...
    def post_run(self, req: BaseChatRequest, result: Any) -> None:
        ...
        No-op for a scheduled run, mirroring pre_run().
