...
class LogfireSmolagentsRunner(SmolagentsRunner):
    ...
    async def run(self, agent: Any, session: Session, requests: list[AgentRequest]) -> AgentReply:
        ...
        Smolagents emits OpenTelemetry spans natively into the global tracer provider that Logfire
        installs at ``init()``; the AK-level span below wraps them and correlates by session.
