...
1. Update `config.yaml` — queue-mode deployments (ECS/Lambda) auto-enable the feature; single-process REST needs an explicit opt-in. The settings are nested inside the existing `session:` block, because the mapping store is the session store's own: add `initiation:` under `session:` with `enabled: true` (required for single-process REST; queue-mode auto-enables), plus an optional `store:` key naming a dotted path to a bring-your-own `MappingStore` subclass. The store's backend follows `session.type` automatically, reusing its connection settings, with the namespace (table/collection name or key prefix) and TTL derived from the session store's own.
