...
11. **Sandbox** — Isolated code/command execution with pluggable providers, workload profiles, policy, and per-user identity
...
**Ask:** Which deployment shape — single-process REST (integration handler in the same process) or queue-based (AWS Lambda / ECS)? Should it apply to all agents or only some (the `agents` list)?

1. Update `config.yaml` — queue-mode deployments (ECS/Lambda) auto-enable the feature; single-process REST needs an explicit opt-in. The settings are nested inside the existing `session:` block, because the mapping store is the session store's own: add `initiation:` under `session:` with `enabled: true` (required for single-process REST; queue-mode auto-enables), plus an optional `agents:` list scoping the tool to named agents (omit = all). There is no mapping-store setting: the Session ID Mapping store is always the session store's own, so a custom one comes from a custom `SessionStore.get_mapping_store()`. The store's backend follows `session.type` automatically, reusing its connection settings, with the namespace (table/collection name or key prefix) and TTL derived from the session store's own.

2. When enabled, the `initiate_conversation(target, prompt, user_id, agent)` system tool is registered on all agents (or only those in `session.initiation.agents`, when set): it creates a fresh session, composes the outbound message by running the agent with `prompt` (the new session's history contains the exchange), and dispatches it for delivery. Inbound platform messages resolve their conversation id through the Session ID Mapping automatically.

   **Step 3 is mandatory, not a refinement.** Auto-enable only guarantees the message reaches the Output Queue — the platform send is the user's override. Without it the tool reports "Conversation initiated." to the agent and the stock response handler drops the message, so the agent claims success while the recipient hears nothing. If the user is not ready to wire the send point, set `session.initiation.enabled: false` rather than leaving the tool advertised.
