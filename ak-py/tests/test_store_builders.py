...
from agentkernel.thread.store.base import _BUILTIN_THREAD_STORES, ThreadStore, ThreadStoreBuilder
from agentkernel.thread.store.in_memory import InMemoryThreadStore
from agentkernel.thread.store.valkey import ValkeyThreadStore
...
def test_thread_builder_valkey_missing_extra_points_at_pip_extra(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "agentkernel.thread.store.valkey", None)
    with patch.object(AKConfig, "get") as mock_get:
        cfg = Mock()
        cfg.thread.type = "valkey"
        mock_get.return_value = cfg
