...
def test_trace_logfire_missing_extra_raises_friendly_import_error(monkeypatch):
    ...
    # Evict the provider module so `from .logfire.logfire import Logfire` re-executes its top-level
    # `import logfire` (another test may have already imported and cached it).
    monkeypatch.delitem(sys.modules, "agentkernel.trace.logfire.logfire", raising=False)
