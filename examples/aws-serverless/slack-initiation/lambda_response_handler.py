"""
Response Handler Lambda — delivers agent replies to Slack.
...
- Ordinary chat replies: channel/thread_ts arrive as custom SQS attributes,
  carried through by lambda_agent_runner.py from the original inbound Slack
...
- INITIATION messages: the agent proactively contacting someone via the
  ...
  The stock ResponseHandler logs a warning and drops these by design — this
  ...
  resolves back to this session (see lambda_request_handler.py's
  resolve_session_id).
"""
...
from agentkernel.aws import ResponseHandler, SQSHandler
from agentkernel.core.initiation import INITIATION_MESSAGE_TYPE, InitiationManager, InitiationMessage
from slack_sdk.web.async_client import AsyncWebClient

logger = logging.getLogger("ak.example.slack_initiation.response_handler")
...
class SlackResponseHandler(ResponseHandler):
    @classmethod
    async def _deliver_initiation(cls, initiation: InitiationMessage) -> str:
        ...
        examples/api/slack-initiation/server.py: opens a DM if the target looks
        like a Slack member id, else posts directly to it as a channel id.
        Returns the posted message's ts — the thread root a reply must be
...
handler = SlackResponseHandler.handle
