...
create_request_handler_deployment_package() {
	...
	if [[ ${1-} != "local" ]]; then
		...
		uv pip install --force-reinstall --no-deps --no-index --target=dist_request_handler/data --find-links ../../../ak-py/dist agentkernel[aws,redis,slack] || true
	fi
...
create_agent_runner_deployment_package() {
	...
	if [[ ${1-} != "local" ]]; then
		...
		uv pip install --force-reinstall --no-deps --no-index --target=dist_agent_runner/data --find-links ../../../ak-py/dist agentkernel[aws,openai,redis] || true
	fi
...
create_response_handler_deployment_package() {
	...
	if [[ ${1-} != "local" ]]; then
		...
		uv pip install --force-reinstall --no-deps --no-index --target=dist_response_handler/data --find-links ../../../ak-py/dist agentkernel[aws,redis,slack] || true
	fi
