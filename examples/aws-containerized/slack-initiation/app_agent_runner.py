"""
Agent Runner (ECS) — runs the two-agent Slack notification flow and carries
...
slack_request_handler.py for where these attributes are attached and
slack_output_consumer.py for where they're read back to deliver the reply.
...
from agentkernel.aws import ECSAgentRunner, SQSHandler
...
class AgentRunner(ECSAgentRunner):
