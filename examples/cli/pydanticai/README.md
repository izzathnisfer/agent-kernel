...
> build with `./build.sh local` against a locally built `ak-py/dist`. The committed `uv.lock` is
> generated by the release tooling once the extra ships, alongside the other examples.
