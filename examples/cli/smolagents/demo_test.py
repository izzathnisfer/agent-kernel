...
async def test_client():
    client = CLIClient("demo_toolcalling.py")
    await client.start()
    try:
        yield client
    finally:
...
async def test_first_question(test_client):
    ...
    Test.compare(test_client.last_agent_response, ["Sri Lanka won the 1996 cricket world cup."], threshold=20)


@pytest.mark.order(2)
async def test_follow_up_question(test_client):
    await test_client.send("Which country hosted the tournament?")
    Test.compare(test_client.last_agent_response, ["Co-hosted by India, Pakistan and Sri Lanka."], threshold=20)
