...
| Sample source | `sample_source/` | Committed markdown (four **original synthetic** detective stories — see the note at the end of this file) the Curator syncs into the bundle. The bundle itself (`sample_bundle/`) is *generated* by the sync and is git-ignored. |
...
The demo defaults to `FileSystemStorage`: the bundle lives at `./sample_bundle/`
(generated by the sync — git-ignored and empty on a fresh checkout) and the sync
source is the committed `./sample_source/` (four original synthetic detective
stories — see the note at the end of this file).
Choosing the backend is the **only** difference between running locally and
against S3.
...
You start on the **consumer** agent, but on a fresh checkout the bundle is empty
— run the **curator** sync (Flow 1) first to populate it, then ask questions.
Switch agents with `!select <name>` and see them with `!list`.

### Flow 1 — import & organize (curator)

```
(consumer) >> !select curator
(curator)  >> Import the source folder and organize it into meaningful concepts.
```

The Curator reads every markdown file under `sample_source/`, splits each into
meaningful OKF concepts, and files them under category subfolders that reflect
*what* they are — `characters/`, `places/`, `incidents/`, `things/`,
`relationships/`, `themes/` — cross-linking related concepts into a navigable
graph. Each `write_concept` regenerates the touched `index.md`, and the Curator
appends a summary to `log.md`.

For a quick **verbatim** import instead, ask it to `sync_source`: that mirrors
the source layout into a `synced/` subtree, timestamp-based and idempotent
(re-runs skip files whose source last-modified time is unchanged).

### Flow 2 — ask (consumer)

```
(curator)  >> !select consumer
(consumer) >> In "The Lighthouse Cipher", what name does Inspector Merrow use for the thief Coral Deveraux?
(consumer) >> Which story features a violinist vanishing on stage, and what was the disappearance a cover for?
```

The Consumer starts at the root `index.md`, walks the tree into `synced/`, and
grounds every answer in the synced documents (citing a document's `resource`
...
(producer) >> Create a summary concept at summaries/lighthouse_cipher.md for "The Lighthouse Cipher", type Summary, then log the change.
```
...
> which is git-ignored scratch. Reset it any time with `rm -rf sample_bundle`
> (the next curator sync repopulates it).
...
The four stories under `sample_source/` are **original synthetic fixtures**
written for this example — invented characters (Inspector Ada Merrow, Dr. Elias
Vane, the thief "the Magpie" / Coral Deveraux) and plots, not derived from any
published work. They exist only to give the Curator a small, realistic
multi-document corpus to sync; no third-party or licensed text is committed here.
Replace them freely with your own markdown via `OKF_SOURCE_DIR`.
