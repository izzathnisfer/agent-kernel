...
| Sample source | `sample_source/` | Committed markdown (the Sherlock Holmes stories) the Curator syncs into the bundle. The bundle itself (`sample_bundle/`) is *generated* by the sync and is git-ignored. |
...
source is the committed `./sample_source/` (the Sherlock Holmes stories).
...
(consumer) >> In "A Scandal in Bohemia", what name does Holmes use for Irene Adler?
(consumer) >> Which story features the Red-Headed League, and what was the league a cover for?
```
...
(producer) >> Create a summary concept at summaries/speckled_band.md for "The Adventure of the Speckled Band", type Summary, then log the change.
```
