...
5. **Privacy by construction.** Private incident records may contain a voluntary contact, while `public_incident_brief` strips contacts and coarsens fine-grained location data before a brief is shared to a public group.
6. **Follow-up is part of the agent lifecycle.** The coordinator can use Agent Kernel's scheduled-task capability to arrange future check-ins instead of relying on a person to remember every unresolved case.
7. **Cross-session community coordination.** Operational incidents/resources are shared across users in the running service, while Agent Kernel session memory remembers each user's current incident/resource/area for conversational continuity.
8. **Works even for a judge without an LLM key.** `demo_scenario.py` exercises the incident ledger, deduplication, verification, matching, confirmation gate, privacy layer, and operational metrics deterministically.
...
| Tool binding | Native tools implement reporting, verification, inventory, matching, status, privacy briefs, and metrics. |
| Session state | Agent Kernel non-volatile session cache remembers each user's active incident/resource/area, while a process-shared community ledger lets different chat sessions coordinate against the same incidents and resources. |
...
| REST/queue pipeline | `api_server.py` runs Agent Kernel `IOHandler` with the in-memory queue pipeline. |
...
| --- | --- |
| **Idea / Use Case Value — 40%** | Real coordination bottleneck; trust-aware deduplication; verification; privacy-safe sharing; human-gated resource matching; four SDGs. |
...
| **End Product / Working Solution — 20%** | No-key end-to-end judge demo, automated tests, CLI entry point, Telegram server, and REST/scheduling server. |
...
### A. Fastest judge path — deterministic end-to-end scenario, no LLM key
...
1. a high-priority flood report;
2. a second, likely duplicate report being merged;
3. verification by a trusted volunteer;
4. registration of a boat and drinking-water offer;
5. ranked resource matching;
6. explicit human confirmation of one match;
7. a privacy-safe public brief; and
8. an operations snapshot.

### B. Run the real multi-agent CLI
...
### D. Run Agent Kernel scheduling + REST pipeline
...
The tests cover priority transparency, duplicate merging, privacy-safe briefs, verification, match proposal semantics, human confirmation, and operational metrics.
...
├── demo_scenario.py          # no-key deterministic judge demo
